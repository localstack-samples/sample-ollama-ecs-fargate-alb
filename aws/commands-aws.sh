...
export SG_ID1=$(aws ec2 create-security-group \
...
export SG_ID2=$(aws ec2 create-security-group \
...
export LB_ARN=$(aws elbv2 create-load-balancer \
...
echo "REACT_APP_LB_NAME=$LB_NAME" >> frontend/chatbot/.env
...
aws ecr get-login-password --region us-east-1 | docker login --username AWS --password-stdin <your_account_number>.dkr.ecr.us-east-1.amazonaws.com

export MODEL_NAME=tinyllama
echo "REACT_APP_MODEL_NAME=$MODEL_NAME" >> frontend/chatbot/.env


docker build --build-arg MODEL_NAME=$MODEL_NAME -t ollama-service .

docker tag ollama-service:latest <your_account_number>.dkr.ecr.us-east-1.amazonaws.com/ollama-service:latest
docker push <your_account_number>.dkr.ecr.us-east-1.amazonaws.com/ollama-service:latest
...
export ECS_TASK_PARN=$(aws iam create-policy \
...
export ECS_TASK_EXEC_PARN=$(aws iam create-policy \
...
aws ecs register-task-definition \
  --family ollama-task \
  --cli-input-json file://task_definition_aws.json
